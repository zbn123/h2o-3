...
class ModelBase(backwards_compatible()):
    ...
    def download_mojo(self, path=".", get_genmodel_jar=False, genmodel_name=""):
        ...
        assert_is_type(path, str)
        assert_is_type(get_genmodel_jar, bool)
        if self.algo not in {"drf", "gbm", "deepwater", "deeplearning", "xgboost", "glrm", "glm", "word2vec"}:
            raise H2OValueError("MOJOs are currently supported for Distributed Random Forest, "
                                "Gradient Boosting Machine, XGBoost, Deep Water, Deep Learning, GLM, GLRM and word2vec models only.")
        if get_genmodel_jar:
            if genmodel_name == "":
